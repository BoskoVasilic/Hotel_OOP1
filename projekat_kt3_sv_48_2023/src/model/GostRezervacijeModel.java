...
public class GostRezervacijeModel extends AbstractTableModel {
	private static final long serialVersionUID = -2197923161756942723L;
	...
	public GostRezervacijeModel(RezervacijaManager rm) {
		this.rm = rm;
	}
	
	@Override
	public int getRowCount() {
		return rm.getRezervacijeZaGosta().size();
	}
	...
	public Object getValueAt(int rowIndex, int columnIndex) {
		if (rowIndex >= rm.getRezervacijeZaGosta().size()) {
			switch (columnIndex) {
			case 0:
				return "";
			case 1:
		...
		Rezervacija r = rm.getRezervacijeZaGosta().get(rowIndex);
		switch (columnIndex) {
		case 0:
			return r.getId();
		case 1:
